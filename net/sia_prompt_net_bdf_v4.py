...
class BiGatedDifferenceFusionV4(nn.Module):
    """BDF-v4: BDF-v2 reliability floor plus learnable temporal residual gamma."""
...
class SiameseSTPromptNetBDFV4(BaseSiameseSTPromptNet):
    def __init__(self, num_classes, input_channels=1, deep_supervision=True):
        ...
        self.st_fusion_bottleneck = BiGatedDifferenceFusionV4(512)


def build_model(window_size=5, image_size=(512, 512), num_classes=1, input_channels=1):
    print(f"[Model] Building Siamese ST-Prompt-Net BDF-v4 | Window: {window_size}, Input: {image_size}")
    return SiameseSTPromptNetBDFV4(
